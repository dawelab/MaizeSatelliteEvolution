...
##data_files<- dir(getwd(), pattern=c( ".*sub_HOR_all_HORknobs_.*blat_bins_nam_groups.*\\.csv$" ".blat_bins_nam_groups.csv"), full.names=FALSE)
data_files<- dir(getwd(), pattern=".*sub_HOR_all_HORknobs_.*blat_bins_nam_groups.*\\.csv$", full.names=FALSE)
`%!in%` = Negate(`%in%`)
...
for(j in 1:length(data_files)){
  ...
  for( p in unique(pattern_out$dir)){
  	...
  	sub<- pattern_out[pattern_out$dir %in% p,] %>% filter(startsWith(as.character(bin), 'chr')) 
	for(b in unique(sub$bin)){
  		...
  		for( d in data_fil){
  			...
  			#HOR_patterns<- select( HOR_patterns_dat dat_out_Z_sub_char, c("out", "Freq"))
...
write.table(string_out, "HORknobs_SHARED_Pattern_String.out", col.names=F, row.names=F, quote=F)
